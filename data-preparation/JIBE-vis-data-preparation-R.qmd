...
for (scenario in c('reference','cycleIntervention')) {
    data$Manchester$volumes[[scenario]] <-list()
    data$Manchester$volumes[[scenario]][["bikePed"]] <- list()
    data$Manchester$volumes[[scenario]][["carTruck"]] <- list()
    for (day in c('saturday','sunday','thursday')) {
        data$Manchester$volumes[[scenario]]$bikePed[[day]] <- list(
          source = paste0('manchester/simulationResults/ForUrbanTransition/',scenario,'/matsim/processed/dailyVolume_bikePed_',day,'.csv'),
      ...
      data$Manchester$volumes[[scenario]][["carTruck"]][[day]] <- list(
          source = paste0('manchester/simulationResults/ForUrbanTransition/',scenario,'/matsim/processed/dailyVolume_carTruck_',day,'.csv'),
          description = "These files contain modelled daily traffic counts by mode and day of week. Number of records equal to number of links/edges in the network.",
          citation = "?",
          variables = list(
            linkId = 'link segment id by direction (out/rtn)',
...
for (scenario in c('reference','cycleIntervention')) {
  for (day in c('saturday','sunday','thursday')) {
    data$Manchester$emissions[[scenario]] <- list(
...
#### Persons
```{r}
  data$Manchester$population[["persons.reference"]] <- list(
    # source = "manchester/synpop/sp_2021/pp_health_2021.csv",
    source = paste0("manchester/simulationResults/ForUrbanTransition/reference/health/03_exposure_and_rr/pp_health_2021.csv",
...
data$Manchester$population[["persons.intervention"]] <- list(
    # source = "manchester/synpop/sp_2021/pp_health_2021.csv",
    source = paste0("manchester/simulationResults/ForUrbanTransition/intervention/health/03_exposure_and_rr/pp_health_2021.csv",
    description = "A generated representation of Greater Manchester's population characteristics, risks and outcomes (synthetic population).",
    citation = "?",
    variables = list(
      personid = "person id", # renamed from id
...
data$Manchester[["trips"]] <- list(
    source = paste0('manchester/simulationResults/ForUrbanTransition/',scenario,'/travel_demand_mito/trips.csv'),
...
For a preliminary visualisation of the Manchester network data, the following is proposed:
...
for (scenario in c('reference')) {
  volumes[[scenario]] <- list()
  for (mode in c('bikePed','carTruck')) {
    file <- paste0('../../../',
        data$Manchester$volumes[[scenario]][[mode]][[day]]$source
      )
    cat(file)
    volumes[[scenario]][[mode]] <- read_csv(file)
    columns <- volumes[[scenario]][[mode]] %>% names()
    counts <- setdiff(columns, c('linkId','edgeId','osmId'))
    volumes[[scenario]][[mode]] <- volumes[[scenario]][[mode]] %>%
      group_by(edgeId) %>%
...
joined_networks <- inner_join(
  ...
  left_join(volumes$reference$bikePed, by = c("edgeID" = "edgeId")) %>%
  left_join(volumes$reference$carTruck, by = c("edgeID" = "edgeId"))
...
# ifelse(joined_networks$path_ref==joined_networks$path_int, 1, 0) %>% table()
##      1 
## 527851 
...
networks$intervention <- networks$intervention %>%
  mutate(scenario = "intervention")

# Append the datasets
full_network <- bind_rows(
  networks$reference %>% as.data.frame(),
  networks$intervention %>% as.data.frame()
)

# Print the first few rows of the appended dataset
print(head(full_network))
```

```{r}
#| output: true
parquet_output_path <- "../../../visualisation/derived_data/parquet/network_manchester_2021.parquet"
output_dir <- dirname(parquet_output_path)
if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE)
}
write_parquet(full_network %>% select(-path) %>% as.tibble(), parquet_output_path)
...
```{r}
#| output: true
full_network <- full_network %>% select(-path)

variables <- names(full_network)
data_types <- sapply(full_network, class)
...
To create a parquet network table in AWS Athena:
...
In the below we define a table for the synthetic population on the Athena jibevisdatabase, that will support subsequent querying. This table uses multi-level partitioning across progressively smaller area scales that we may draw upon in subsequent linkage queries when users click on area geometries: LADs, MSOAs, LSOAs and OAs.  For example, a user might click on an MSOA and 3D curved lines might be drawn connecting the home and work LSOA locations for synthetic residents, colourised by some relevant statistic, grouping or covariate.  Or interactive gender and age group stratified box  plots of a continuous variable like totalActivityTime or mmetHr_cycle could be displayed.  Or a scatter plot of totalTravelTime by totalActivity time.  And so forth.   Separating population statistics from the geometry could support development of post hoc interactive queries like these.
...
For LSOAs, we will also use this layer to depict 2011 Index of Multiple Deprivation (2019; IMD).  To do this, we need to join on the correspondence codes for 2021 to 2011 LSOAs, and then join on the IMD data.
...
```{r}
#| output: true
trips.reference <- read_csv(paste0('../../../',data$Manchester$trips$source))

parquet_output_path <- "../../../visualisation/derived_data/parquet/trips_manchester.parquet"
output_dir <- dirname(parquet_output_path)
if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE)
}
write_parquet(trips, parquet_output_path)
...
```{r}
#| output: true
variables <- names(trips)
data_types <- sapply(trips, class)
