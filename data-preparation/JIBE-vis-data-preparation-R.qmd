...
data$Manchester$population[["persons.intervention"]] <- list(
    # source = "manchester/synpop/sp_2021/pp_health_2021.csv",
    source = paste0("manchester/simulationResults/ForUrbanTransition/cycleIntervention/health/03_exposure_and_rr/pp_health_2021.csv"),
    description = "A generated representation of Greater Manchester's population characteristics, risks and outcomes (synthetic population).",
    citation = "?",
    variables = list(
      personid = "person id", # renamed from id
...
#| output: true
# Add a scenario column to each dataset
synpop$persons.reference <- synpop$persons.reference %>%
  mutate(scenario = "reference")

synpop$persons.intervention <- synpop$persons.intervention %>%
  mutate(scenario = "intervention")

# Append the datasets
synpop$persons <- bind_rows(
  synpop$persons.reference %>% as.data.frame(),
  synpop$persons.intervention %>% as.data.frame()
)

# Print the first few rows of the appended dataset
print(head(synpop$persons))
```
