...
data$Melbourne$parameters$linkage_areas  <- list(
  SA1 = list(
    ...
    linkage = list(
      lookup = list(
        source = paste0('../../../',data$Melbourne$linkage$SA1_LGA$source),
        select = data$Melbourne$linkage$SA1_LGA$variable %>% names(),
        by = 'SA1_MAINCODE_2016'
      ),
      disadvantage = list(
        source = paste0('../../../',data$Melbourne$linkage$SEIFA_SA1$source),
  ...
  SA2 = list(
    ...
    linkage = list(
      disadvantage = list(
        source = paste0('../../../',data$Melbourne$linkage$SEIFA_SA2$source),
  ...
  lGA = list(
    ...
    linkage = list(
      disadvantage = list(
        source = paste0('../../../',data$Melbourne$linkage$SEIFA_LGA$source),
  ...
  for (scenario in data[[city]]$parameters$scenarios) {
    data[[city]]$population[[paste0("persons.",scenario)]] <- list(
      ...
      variables = list(
        ...
        income = paste0("in ", currency[[city]]),  # City-dependent currency
...
for (city in cities) {
  ...
  data[[city]]$population[["dwellings"]] <- list(
    ...
    variables = list(
      ...
      monthlyCost = paste0("Rent (", currency[[city]], "/month)"),  # City-dependent currency
...
for (city in cities) {
  ...
  derived_network_path <- paste0("../../../visualisation/derived_data/FlatGeobufs/", city_lower,"_4326.fgb")
...
for (city in cities) {
  city_lower <- tolower(city)
  derived_network_path <- paste0("../../../visualisation/derived_data/FlatGeobufs/", city_lower, "_network_bsd_scenarios_4326.fgb")
  pmtiles_network_path <- paste0("../../../visualisation/derived_data/PMTiles/", city_lower, "_network_bsd_scenarios_4326.pmtiles")
...
for (city in cities) {
  ...
  if (city == 'Manchester') {
    ...
    sa1_geoms <- st_read(paste0("../../../",data$Melbourne$areas$SA1$source), 
...
for (city in cities) {
  if (city == 'Manchester') {
    ...
    mb_geoms <- st_read(paste0("../../../", data$Melbourne$areas$MB$source), 
    ...
    lga_lookup <- read_csv(paste0('../../../', data$Melbourne$areas$MB_LGA_linkage$source), show_col_types = FALSE)
...
for (city in cities) {
  ...
  if (!is.null(data[[city]]$parameters$linkage_areas)) {
    ...
    for (area_type in names(linkage_areas)) {
      ...
      spatial_data_to_fgb(
        paste0('../../../', data[[city]]$areas[[area_type]]$source),
        paste0('../../../', data[[city]]$areas[[area_type]]$output),
